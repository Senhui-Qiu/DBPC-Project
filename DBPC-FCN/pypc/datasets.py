...
class CIFAR10(datasets.CIFAR10):
    ...
    def __getitem__(self, index):
        data, target = super().__getitem__(index)
        # data = _to_vector(data)
        data = _to_vector_multi_channels(data)
        target = _one_hot(target)
        if self.expand_size > 1:
            target = self.expand_lable(target, self.expand_size)
        if self.scale is not None:
...
def accuracy_1(pred_labels, true_labels):
    correct = 0
    if torch.argmax(pred_labels[:]) == torch.argmax(true_labels[:]):
        correct += 1
    return correct
...
def _to_vector_multi_channels(batch):
    return batch.reshape(1, -1).squeeze()