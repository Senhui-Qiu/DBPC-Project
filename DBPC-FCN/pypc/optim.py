...
def get_optim(params, cf):
    if cf.optim_id is "Adam":
        return Adam(params, weight_share=cf.weight_share, weight_no_share_num=cf.weight_no_share_num, du_beta=cf.du_beta, du_alpha=cf.du_alpha, lr=cf.du_lr, q_lr=cf.q_lr, batch_scale=cf.optim_batch_scale, grad_clip=cf.optim_grad_clip, weight_decay=cf.optim_weight_decay)
    elif cf.optim_id is "SGD":
        return SGD(params, lr=cf.du_lr, q_lr=cf.q_lr, batch_scale=cf.optim_batch_scale, grad_clip=cf.optim_grad_clip, weight_decay=cf.optim_weight_decay)
    else:
        raise ValueError(f"{cf.optim_id} not a valid optimizer ID")
...
class Optimizer(nn.Module):
    ...
    def decay_weights_rev(self, param):
        if self.weight_decay is not None:
            if self.weight_share == True:
                param.grad_rev["weights_rev"] = param.grad_rev["weights_rev"] - self.weight_decay * param.weights.T
            else:
                param.grad_rev["weights_rev"] = param.grad_rev["weights_rev"] - self.weight_decay * param.weights_rev.T
...
class Adam(Optimizer):
    def __init__(
        ...
        self.c_w_rev = [torch.zeros_like(param.weights.T) for param in self._params]
        self.v_b_rev = [torch.zeros_like(param.bias) for param in self._params]
        self.v_w_rev = [torch.zeros_like(param.weights.T) for param in self._params]
            ...
            for p, param in enumerate(self._params):
                ...
                if self.weight_share == True:
                    if p >= (self.n_params-self.weight_no_share_num):
                        param.weights_rev += (_lr * delta_w).T
                    else:
                        param.weights += (self.du_alpha * _lr * delta_w).T
                else:
                    param.weights_rev += (self.du_alpha * _lr * delta_w).T
